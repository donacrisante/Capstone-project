...
export default function Overview({ entries }) {
  ...
  const totalEmissionPerTransport = (transport, fuel, month) =>
    Math.floor(
      entries
        .filter((entry) => {
  ...
  totalEmissionPerTransport("car", "petrol", monthIndex)
  );
  const carDiesel = months.map((_, monthIndex) =>
  totalEmissionPerTransport("car", "diesel", monthIndex)
  );
  const carHybrid = months.map((_, monthIndex) =>
  totalEmissionPerTransport("car", "hybrid", monthIndex)
  );
  const carElectricStrommix = months.map((_, monthIndex) =>
  totalEmissionPerTransport("car", "electric-strommix", monthIndex)
  );
  const carElectricRenewable = months.map((_, monthIndex) =>
  totalEmissionPerTransport("car", "electric-renewable", monthIndex)
  );
  const train = months.map((_, monthIndex) =>
  totalEmissionPerTransport("train", null, monthIndex)
  );
  const plane = months.map((_, monthIndex) =>
  totalEmissionPerTransport("plane", null, monthIndex)
  );
  const bicycle = months.map((_, monthIndex) =>
  totalEmissionPerTransport("bycicle", null, monthIndex)
  );
